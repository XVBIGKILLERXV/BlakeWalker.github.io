/* Blake Walker portfolio — live Discord presence card
...
(() => {
  ...
  const USER_ID = String(window.BLAKE_DISCORD_USER_ID || '').trim();
  ...
  // A real Discord snowflake is numeric. If it has not been configured yet,
  // show a small setup state instead of silently failing.
  if (!/^\d{15,22}$/.test(USER_ID)) {
    section.classList.add('is-live', 'presence--setup');
    card.innerHTML = '<div class="presence__setup"><strong>Discord presence</strong><span>Add your numeric Discord User ID in <code>discord-config.js</code> to make this card live.</span></div>';
    return;
  }
